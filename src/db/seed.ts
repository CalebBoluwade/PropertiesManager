...
async function seed() {
  ...
  // ── Property type fields (Land) ─────────────────────────────────────────────
  await db.insert(propertyTypeFields).values([
    { propertyTypeId: land.id, name: "Plot Size",       key: "plotSize",       fieldType: "number", required: false },
    { propertyTypeId: land.id, name: "Title Document",  key: "titleDocument",  fieldType: "text",   required: false },
  ]).onConflictDoNothing();
  ...
  for (const lease of leaseRows) {
    for (let m = 2; m >= 0; m--) {
      ...
      obligationRows.push({
        ...
        amountPaid: isPast ? lease.monthlyRent : lease.monthlyRent * 0.5,
  ...
  // ── Payments (one per paid/partial obligation) ───────────────────────────────
